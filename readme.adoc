...
. NodeJS +
tested with v9 et v10

 $ npm install

. Ruby et Bundler (testé avec Ruby 2.5)
...
The node module provides a sample project in its sample/ directory.

Initialize the JavaScrip project, and add the dependency:

 $ npm init -y
 $ npm install @sewatech/bespoke-course
 $ cp -r node_modules/@sewatech/bespoke-course/sample/* ./
...
The best unit for setting the size of images is *rem*.
